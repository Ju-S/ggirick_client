...
export default function OrganizationMembersModal({
      ...
      .map((org) => {
        const filteredDepartments = org.departments
          ?.map((dept) => {
            ...
            return filteredEmployees?.length ? { ...dept, employees: filteredEmployees } : null;
          })
          ?.filter(Boolean);
        return filteredDepartments?.length ? { ...org, departments: filteredDepartments } : null;
      })
      ?.filter(Boolean);
  ...
  return (
    <BaseModal isOpen={open} onClose={onClose} title={title}>
      ...
      {/* 조직도 계층 */}
      <div className="max-h-[400px] overflow-y-auto rounded-lg bg-base-100 p-3 space-y-3 border border-base-200">
        {filteredStructure.length === 0 ? (
          ...
          filteredStructure.map((org) => (
            <details key={org.code} className="collapse collapse-arrow bg-base-200 rounded-lg" open>
              <summary className="collapse-title font-bold text-base">🏢 {org.name}</summary>
                          ...
                          return (
                            ...
                            >
                              ...
                              {showExistingMark && isExisting && (
                                <span className="badge badge-outline badge-sm">기존</span>
                              )}
                            </label>
                          );
                        })
                      ) : (
                        <p className="text-xs text-gray-400 italic px-2 py-1">등록된 직원이 없습니다.</p>
                      )}
                    </div>
                  </details>
                ))}
