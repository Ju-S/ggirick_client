...
import {calendarGroupMemberListAPI, deleteAPI, putGroupMemberAPI} from "@/api/calendar/calendarGroupAPI.js";
import useCalendarStore from "@/store/calendar/useCalendarStore.js";

export default function CalendarSidebar() {
    const navigate = useNavigate();
    const groupItems = useCalendarGroupStore(state => state.list); // 사용자가 속한 그룹
    ...
    const [calendarGroupMembers, setCalendarGroupMembers] = useState([]);
    const [selectedGroup, setSelectedGroup] = useState(1);
    const {setModalOpen, setNewEvent} = useCalendarStore();
    ...
    const handleAddSchedule = () => {
        setNewEvent({title: "", startAt: null, endAt: null, description: "", recurrence: "none", color: "#dddddd"});
        setModalOpen(true);
    }
        ...
        <div className="w-full bg-base-100 border-r border-base-300 h-full flex flex-col">
                        ...
                        groupItems
                            .map(group => (
                                <div
                                    ...
                                    onClick={() => navigate(`/calendar?groupId=${group.id}`)}
                                >
                                    {/* 그룹명 */}
                                    ...
                                    <div
                                        ...
                                        {selectedEmployee?.id === group.owner && (
                                            <>
                                                {/* 구성원 추가 버튼 */}
                                                <button
                                                    ...
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        calendarGroupMemberListAPI(group.id)
                                                            .then(resp => setCalendarGroupMembers(resp.data))
            ...
            {selectedEmployee &&
                <FilteredOrganizationMemberModal
                    ...
                    selectedMemberIds={calendarGroupMembers}
