...
export default function FilteredOrganizationMemberModal({
                                                          open,
                                                          onClose,
                                                          title = "조직도에서 멤버 선택",
                                                          selectedMemberIds = [],
        ...
        const fetchStructure = async () => {
            try {
                ...
                const filteredData = res.data
                    .filter(org => !selectedOrganizationCodes.length || selectedOrganizationCodes.includes(org.code))
                    .map(org => {
                        const filteredDepartments = org.departments
                            ?.map(dept => {
                                const filteredEmployees = dept.employees?.filter(
                                    emp => !exclusiveMemberIds.includes(emp.id)
                                );
                ...
                setOrganizationStructure(filteredData || []);
            } catch (err) {
        ...
        <BaseModal isOpen={open} onClose={onClose} title={title}>
            ...
            <div className="max-h-[500px] overflow-y-auto rounded-lg bg-base-100 p-3 space-y-3 border border-base-200">
                {filteredStructure.length === 0 ? (
                    <div className="text-center text-base-content-400 py-6">
                        ...
                        >
                            ...
                            <div className="collapse-content space-y-2">
                                {org.departments?.map((dept) => (
                                    ...
                                    >
                                        <summary className="collapse-title text-sm font-semibold text-base-content-700">
                                            📁 {dept.name}
                                        </summary>
                                ...
                                hover:bg-base-200 ${
                                  ...
                                  기존
                                                ...
                                                <p className="text-xs text-base-content-400 italic px-2 py-1">
                                                    등록된 직원이 없습니다.
                                                </p>
                                            )}
                                        </div>
            ...
            <div className="flex justify-end gap-3 mt-4">
                ...
                <button
                    ...
                    onClick={() => onSave?.(selectedEmployeeDetails)}
                >
                    선택 완료
                </button>
            </div>
